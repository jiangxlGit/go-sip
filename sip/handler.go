...
func handlerInvite(req *sip.Request, tx *sip.Transaction) {
	...
	Logger.Info("开始处理invite请求", zap.Any("u.DeviceID", u.DeviceID))
	ack, err := InviteFunc(u.DeviceID)
	if err != nil {
		Logger.Error("invite 失败", zap.Error(err))
	}

	Logger.Info("收到服务端 invite 反馈 请求", zap.Any("ack", ack), zap.Any("ack.zlmIp", ack.ZlmIp), zap.Any("ack.zlmPort", ack.ZlmPort))
