...
func HSetStruct_2[T any](hash_key, key string, val T) error {
	rdb := GetRedisClientByName("wvp_2")
...
func ZAdd_2(key string, items []model.TimeItem, expiration time.Duration) error {
	rdb := GetRedisClientByName("wvp_2")
	...
	for _, item := range items {
		...
		if err != nil {
			Logger.Error("TimeItem 序列化失败", zap.Error(err))
			return err
		}
		zMembers = append(zMembers, redis.Z{
			Score:  float64(item.End), // 用 End 作为 score
...
func ZRangeByScore_2(key, min, max string) ([]model.IpcPlaybackRecordData, error) {
	rdb := GetRedisClientByName("wvp_2")
...
func ZRangeByScoreItem_2(key string, min, max string) ([]model.TimeItem, error) {
	rdb := GetRedisClientByName("wvp_2")
	...
	result := make([]model.TimeItem, 0, len(rawMembers))
	for _, raw := range rawMembers {
		var item model.TimeItem
		if err := json.Unmarshal([]byte(raw), &item); err != nil {
			Logger.Error("TimeItem 反序列化失败", zap.String("raw", raw), zap.Error(err))
			continue
		}
		result = append(result, item)
	}
...
func ZGetMaxScoreItem_2(key string) (*model.TimeItem, float64, error) {
	rdb := GetRedisClientByName("wvp_2")

	res, err := rdb.ZRevRangeWithScores(ctx, key, 0, 0).Result()
	if err == redis.Nil {
		return &model.TimeItem{}, 0, nil
	}
	if err != nil {
		Logger.Error("redis zrevrangewithscores 错误", zap.Error(err))
		return nil, 0, err
	}
	if len(res) == 0 {
		return nil, 0, nil // 无数据
	}

	var item model.TimeItem
	if err := json.Unmarshal([]byte(res[0].Member.(string)), &item); err != nil {
		Logger.Error("TimeItem 反序列化失败", zap.String("raw", res[0].Member.(string)), zap.Error(err))
		return nil, 0, err
	}

	return &item, res[0].Score, nil
}
...
func HSet_4(hash_key string, key string, val string) error {
	rdb := GetRedisClientByName("wvp_4")
	err := rdb.HSet(ctx, hash_key, key, val).Err()
	if err != nil {
		Logger.Error("redis hset 错误")
	}
